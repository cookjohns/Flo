...
let gradient = CGGradientCreateWithColors(CGColorSpaceCreateDeviceRGB(),
                                          ...
                                          [0, 0.51, 1])
CGContextDrawLinearGradient(context, gradient, CGPointMake(40, 40), CGPointMake(100, 160), [])
CGContextRestoreGState(context)

// create a transform for inner dark gold circle (scale, translate right and down)
var transform = CGAffineTransformMakeScale(0.8, 0.8)
transform = CGAffineTransformTranslate(transform, 15, 30)

medallionPath.lineWidth = 2.0

// apple transform to path
medallionPath.applyTransform(transform)
medallionPath.stroke()

// upper (front) side ribbon (blue)
